...
<body style="background-color:#fff;" class="bodyContent">
	<div class="content">
		...
		<c:if test="${only_show ne 1}">
			<div>   
				<form id="uploadBindCardImgForm">
					   <c:if test="${type eq 1}">
					   		<label for="photoImg1" class="from_label">银行卡前照：</label>
					   </c:if>  
					   <c:if test="${type eq 2}">
					   		<label for="photoImg2" class="from_label">银行卡后照：</label>
					   </c:if>
		               <input type="file" id="file" name="file" accept=".jpg,.jpeg,.png,.gif"> 
		               <input style="width:45px;" type="button" value="上传" onclick="doUpload()" id="btn_submit_bindCardImg"/>
			    </form> 
			 </div>
		</c:if>
	...
	<script type="text/javascript">
		...
		function doUpload() {
			$("#btn_submit_bindCardImg").attr("disabled", true); 
			//获取文件的属性
	        ...
	        if(a[0].size>4*1024*1024){
	        	...
	        	if((fileType=="jpg")||(fileType=="jpeg")||(fileType=="gif")||(fileType=="png")){
	        		 var formData = new FormData($( "#uploadBindCardImgForm" )[0]); 
	        		 layer.load();
	    		     $.ajax({  
	    		          url: 'fileUpload!uploadBindCardImg.do?type='+type+'&id='+userId ,  
	    		          type: 'POST',  
	    		          data: formData,  
	    		          async: false,  
	    		          cache: false,  
